...
# Page config
st.set_page_config(page_title="SHA — Bharat’s Full Stack Assistant", page_icon="💻", layout="centered")
...
<style>
  ...
  .stButton>button, button[kind="primary"] {
    background-color:#7F5AF0!important; color:#FFF!important;
...
store    = FAISS.load_local(vector_store_path, OpenAIEmbeddings(openai_api_key=OPENAI_API_KEY), allow_dangerous_deserialization=True)
...
# Handlers
def handle_fun(q):
    if "food" in q:     return "Bharat runs on Java, JSON, and weekend biryani—strictly in that order."
...
def handle_company(q):
    ...
    if any(x in q for x in ["jpmorgan","jpmc"]):
        return "At JPMorgan Chase, he built secure microservices and React apps (2023–2024)."
    if "dentsu" in q:
...
def handle_education(q):
    ...
    if "certification" in q:
        return "Certifications: AWS Developer, Oracle Java Programmer, Power BI Analyst."
    return None
